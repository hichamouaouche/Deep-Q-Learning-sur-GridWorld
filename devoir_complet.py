"""
DEVOIR : Entraînement Double DQN + Génération Complète des Graphiques
═══════════════════════════════════════════════════════════════════════
Fusion complète : devoir.py + train_and_visualize.py
...
class GridWorld:
    """Simple 4x4 grid environment avec agent, objectif et obstacle."""
    ...
    def reset(self):
        """Reset l'agent à la position de départ."""
        self.agent_pos = (0, 0)
        self.goal_pos = (3, 3)
    ...
    def get_state(self):
        """Retourne l'état sous forme de vecteur aplati."""
    ...
    def step(self, action):
        ...
        - Récompense +10 si objectif atteint
        - Récompense -5 si obstacle touché
        - Récompense -1 pour chaque mouvement normal
        """
        x, y = self.agent_pos
        dx, dy = MOVES[action]
...
class DoubleDQNAgent:
    """
    Agent Double DQN avec deux réseaux :
    - online_model   : Sélectionne l'action optimale
    - target_model   : Évalue la Q-value de cette action
    """
    ...
    def update_target_model(self):
        """Copie les poids du online au target."""
    ...
    def act(self, state):
        """Choisit une action avec stratégie ε-Greedy."""
        if np.random.rand() <= self.epsilon:
            return random.randrange(self.action_size)  # Exploration

        q_values = self.online_model.predict(np.array([state]), verbose=0)
        return int(np.argmax(q_values[0]))  # Exploitation

    def replay(self):
        """
        Experience Replay avec Double DQN :
        1. Online network choisit la meilleure action
        2. Target network évalue cette action
        3. Entraîne le online network
        """
        ...
        for i in range(BATCH_SIZE):
            if dones[i]:
                ...
                # Sélection avec online, évaluation avec target
...
def train_double_dqn():
    """
    Entraîne l'agent Double DQN et sauvegarde les métriques.
    Retourne : (rewards, epsilons, steps, agent, env)
    """
    env = GridWorld()
    ...
    print("=" * 70)
    print("ENTRAÎNEMENT DOUBLE DQN - GridWorld 4x4".center(70))
    print("=" * 70)
...
def plot_training_curve(rewards):
    """Graphique 1 : Courbe d'entraînement avec moyenne mobile."""
...
def plot_epsilon_decay(epsilons):
    """Graphique 2 : Décroissance d'Epsilon (taux d'exploration)."""
    fig, ax = plt.subplots(figsize=(12, 6))
...
def plot_steps_per_episode(steps):
    """Graphique 5 : Nombre de pas par épisode (efficacité)."""
    fig, ax = plt.subplots(figsize=(12, 6))
...
def plot_gridworld_visualization(env, agent):
    """Graphique 6 : Visualisation GridWorld + Heatmap Q-values."""
...
def plot_comparison_metrics(rewards, steps):
    """Graphique 7 : Dashboard complet avec statistiques."""
...
def create_summary_report(rewards, epsilons, steps):
    """Crée un fichier texte résumé des résultats d'entraînement."""
